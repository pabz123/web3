...
$stmt = $conn->prepare("
    SELECT 
        ...
        employers.company_website,
        employers.company_address,
    ...
    if (isset($_FILES['company_logo']) && $_FILES['company_logo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExt, $allowedExts) && $_FILES['company_logo']['size'] <= 5000000) {
            ...
            if (move_uploaded_file($_FILES['company_logo']['tmp_name'], $targetPath)) {
                ...
                if (!empty($profile['company_logo']) && file_exists(__DIR__ . '/..' . $profile['company_logo'])) {
                    unlink(__DIR__ . '/..' . $profile['company_logo']);
                }
            }
        }
    }
    ...
    try {
        ...
        if ($employerExists) {
            $stmt = $conn->prepare("
                ...
                WHERE user_id = ?
            ");
            $stmt->bind_param('sssssssi', $companyName, $companyLogo, $companyDescription, $companyWebsite, $companyAddress, $companySize, $industry, $userId);
        } else {
            $stmt = $conn->prepare("
                INSERT INTO employers (user_id, company_name, company_logo, company_description, company_website, company_address, company_size, industry)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->bind_param('isssssss', $userId, $companyName, $companyLogo, $companyDescription, $companyWebsite, $companyAddress, $companySize, $industry);
        }
        $stmt->execute();
        $stmt->close();
        ...
        <form method="POST" enctype="multipart/form-data" class="profile-form">
            ...
            <div class="profile-picture-section">
                ...
                <div class="picture-upload-container">
                    <img id="logo-preview" src="<?= htmlspecialchars($profile['company_logo'] ?? '/uploads/company_logos/default-company.png') ?>" alt="Company Logo" class="profile-picture-preview">
